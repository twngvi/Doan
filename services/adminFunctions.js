...
function convertHtmlToDocContent(html, body) {
  try {
    ...
    // Process block markers first to preserve structure generated by editor
    const markerConverted = convertMarkerBlocksToDocContent(content, body);
    if (markerConverted) {
      Logger.log("✅ Marker-based conversion completed");
      return true;
    }
...
/**
 * Convert marker syntax ([[NOTE]], [[CODE:JS]], [[/NOTE]]...) to Google Doc paragraphs
 * Preserves line breaks and block structure.
 * @param {string} htmlContent
 * @param {Body} body
 * @returns {boolean}
 */
function convertMarkerBlocksToDocContent(htmlContent, body) {
  ...
  const hasMarkers = /\[\[(\/)?([A-Z]+(?::[^\]]+)?)\]\]/i.test(normalized);
  ...
  // Preserve marker text exactly (e.g. [[NOTE]], [[/NOTE]], [[CODE:PYTHON]])
  // and keep paragraph structure by writing each non-empty line as-is.
...
/**
 * Lấy thông tin topic để chỉnh sửa
 * @param {string} topicId - ID của topic cần edit
 * @returns {object} - {success, topic, content, message}
 */
function getTopicForEdit(topicId) {
  try {
    ...
    const contentDocIdIndex = headers.indexOf("contentDocId");
    const contentDocUrlIndex = headers.indexOf("contentDocUrl");

    if (topicIdIndex === -1) {
      return { success: false, message: "Sheet Topics không có cột topicId" };
    }
    ...
    // Build topic object
    const topic = {
      topicId: topicRow[topicIdIndex] || "",
      title: topicRow[titleIndex] || "",
      description: descriptionIndex !== -1 ? (topicRow[descriptionIndex] || "") : "",
      category: categoryIndex !== -1 ? (topicRow[categoryIndex] || "") : "",
      order: orderIndex !== -1 ? (topicRow[orderIndex] || "") : "",
      contentDocId: contentDocIdIndex !== -1 ? (topicRow[contentDocIdIndex] || "") : "",
      contentDocUrl: contentDocUrlIndex !== -1 ? (topicRow[contentDocUrlIndex] || "") : ""
    };

    Logger.log("Found topic: " + JSON.stringify(topic));

    // Get content from Google Doc if available
    let content = "";
    if (topic.contentDocId) {
      try {
        const contentResult = getTopicContentByDocId(topic.contentDocId);
        if (contentResult && contentResult.success) {
          content = contentResult.content || "";
        }
      } catch (docError) {
        Logger.log("⚠️ Could not load doc content: " + docError.toString());
      }
    }
    ...
    return {
      ...
      topic: topic,
      content: content,
      message: "Đã tải thông tin topic thành công"
    };
...
/**
 * Cập nhật nội dung topic (update cả thông tin và doc)
 * @param {object} topicData - Dữ liệu topic cần cập nhật
 * @returns {object} - {success, docId, docUrl, message}
 */
function updateTopicContent(topicData) {
  try {
    Logger.log("=== UPDATE TOPIC CONTENT ===");
    Logger.log("Topic data received: " + JSON.stringify(topicData));
    ...
    const isEditMode = topicData.isEditMode === true;
    const requireExistingDoc = topicData.requireExistingDoc === true;

    // Get topic from sheet to find docId
    const sheet = getSheet("Topics");
    if (!sheet) {
      return { success: false, message: "Không tìm thấy sheet Topics" };
    }
    ...
    for (let i = 1; i < data.length; i++) {
      if (data[i][topicIdIndex] === topicData.topicId) {
        rowIndex = i + 1; // 1-based index for sheet
        if (contentDocIdIndex !== -1) {
          existingDocId = data[i][contentDocIdIndex] || "";
        }
        if (contentDocUrlIndex !== -1) {
          existingDocUrl = data[i][contentDocUrlIndex] || "";
        }
        break;
      }
    }
    ...
    if (rowIndex === -1) {
      return { success: false, message: "Không tìm thấy topic với ID: " + topicData.topicId };
    }

    // Prefer docId sent from client in edit mode to ensure update đúng doc hiện tại
    const requestedDocId = (topicData.contentDocId || "").toString().trim();
    if (isEditMode && requestedDocId) {
      existingDocId = requestedDocId;
      Logger.log("Using requested docId from client: " + existingDocId);
    }

    Logger.log("Found topic at row: " + rowIndex);
    Logger.log("Existing Doc ID: " + existingDocId);
    ...
    if (existingDocId) {
      // Update existing doc
      const updateResult = updateTopicDocument(existingDocId, topicData.content);
      if (!updateResult.success) {
        return updateResult;
      }
      docUrl = updateResult.docUrl || existingDocUrl;
    } else {
      if (isEditMode && requireExistingDoc) {
        return {
          success: false,
          message: "Bài học này chưa có contentDocId để cập nhật. Vui lòng kiểm tra dữ liệu bài học trước khi chỉnh sửa."
        };
      }
      // Create new doc if none exists
      const createResult = createTopicDocument(topicData.title, topicData.content);
      if (!createResult.success) {
        return createResult;
      }
      docId = createResult.docId;
      docUrl = createResult.docUrl;
    }

    // Update topic info in sheet
    if (!isEditMode) {
      if (titleIndex !== -1) {
        sheet.getRange(rowIndex, titleIndex + 1).setValue(topicData.title);
      }
      if (descriptionIndex !== -1) {
        sheet.getRange(rowIndex, descriptionIndex + 1).setValue(topicData.description || "");
      }
      if (categoryIndex !== -1) {
        sheet.getRange(rowIndex, categoryIndex + 1).setValue(topicData.category);
      }
      if (orderIndex !== -1) {
        sheet.getRange(rowIndex, orderIndex + 1).setValue(topicData.order || 999);
      }
    }
    if (contentDocIdIndex !== -1 && docId) {
      sheet.getRange(rowIndex, contentDocIdIndex + 1).setValue(docId);
    }
    if (contentDocUrlIndex !== -1 && docUrl) {
      sheet.getRange(rowIndex, contentDocUrlIndex + 1).setValue(docUrl);
    }
    if (updatedAtIndex !== -1) {
    ...
    return {
      success: false,
      message: "Lỗi cập nhật topic: " + error.toString()
    };
  }
}

/**
 * Cập nhật nội dung Google Doc đã tồn tại
 * @param {string} docId - ID của document cần cập nhật
 * @param {string} htmlContent - Nội dung HTML mới
 * @returns {object} - {success, docUrl, message}
 */
function updateTopicDocument(docId, htmlContent) {
  try {
    Logger.log("=== UPDATE TOPIC DOCUMENT ===");
    Logger.log("Doc ID: " + docId);

    // Open existing document
    const doc = DocumentApp.openById(docId);

    // Get the body and update content
    const body = doc.getBody();

    // Convert HTML to Doc content (clears and rebuilds)
    convertHtmlToDocContent(htmlContent, body);
    ...
    Logger.log("❌ Error updating document: " + error.toString());
    return {
      success: false,
      docUrl: "",
      message: "Lỗi cập nhật document: " + error.toString()
    };
  }
}
